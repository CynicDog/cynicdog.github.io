...
Each request then passes through a minimal set of HTTP filters. The only one we use is `envoy.filters.http.router`, which routes requests based on the attached `RouteConfiguration`.
