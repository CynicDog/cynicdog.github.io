...
<iframe
    ...
    title="Unfiltered NYC Subway Stops"
></iframe>
...
Now we apply a **functional filter** using our `expect_or_drop` decorator to only keep stops that have a `parent_station`. This is a **declarative approach**—we describe *what* to keep, not *how* to iterate and remove rows.
...
<iframe
    ...
    height="600px"
    style={{border:"1px solid #ccc"}}
    title="Filtered NYC Subway Stops"
></iframe>
...
## Bronze → Silver → Gold: Declarative Medallion Architecture

With declarative pipelines, our **medallion architecture** becomes obvious and reproducible:

* **Bronze**: Raw, unprocessed data
* **Silver**: Cleaned and enriched tables
* **Gold**: Aggregated, analytics-ready tables

Each stage is **self-contained** and described by decorators. The execution engine reads the metadata encoded in the decorators and handles persistence, temporary views, and data quality enforcement automatically.
