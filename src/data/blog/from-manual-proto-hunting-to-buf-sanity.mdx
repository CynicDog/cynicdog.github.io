...
>
> You can find the project repository at: [CynicDog/pokedex-grpc](https://github.com/CynicDog/pokedex-grpc)
...
So with protobuf, development kinda starts upside down — or head-first. You don’t write Python code first; instead, you start by defining your APIs in a `.proto` file. There, you describe a `service` — kind of like function signatures if you're from the Java world — along with the `message` types (UDTs) that your API takes and returns.

In my case, I was following the tutorial, and I had a proto file called [`pokedex.proto`](https://github.com/CynicDog/pokedex-grpc/blob/main/proto/pokeapi/v1/pokedex.proto). 

[At the bottom](https://github.com/CynicDog/pokedex-grpc/blob/d284c1a66c9288a485fbc9215721734ae14c77ec/proto/pokeapi/v1/pokedex.proto#L20) of that file, there’s a message type that pulls in a reference to another proto — `pokemon/v1/types.proto`. That one lives out in the wild on the Buf Registry: [pixelperfect/gametypes](https://buf.build/pixelperfect/gametypes/docs/main:pokemon.v1)
...
After that, I added a [`buf.gen.yaml`](https://github.com/CynicDog/pokedex-grpc/blob/main/proto/buf.gen.yaml) to tell `buf` how to generate Python code from the protos. This includes both the regular protobuf classes and the gRPC-specific server/client interfaces.
...
The server ([`grpc_server.py`](https://github.com/CynicDog/pokedex-grpc/blob/main/grpc_server.py)) implements the `PokedexService` from our `pokedex.proto`. It receives a Pokémon name (like "Pikachu") and responds with a `Pokemon` message — populated using types from the external schema we pulled in earlier (`pixelperfect/gametypes`).

#### Client-side

The client ([`grpc_client.py`](https://github.com/CynicDog/pokedex-grpc/blob/main/grpc_client.py)) connects to `localhost:50051`, sends a `GetPokemonRequest`, and prints the response from the server. It uses the generated stub class `PokedexServiceStub` to handle the request/response logic.
...
There are experts who know far better than me about `uv`, but simply put: it uses Rust to deliver fast, reliable Python environment and dependency management with deterministic locking that prevents version conflicts and ensures repeatable installs.
