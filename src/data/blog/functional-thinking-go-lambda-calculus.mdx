---
title: "Functional Thinking in Go: Insights from Lambda Calculus"
author: eunsang
pubDatetime: 2025-10-09T00:00:00Z
modDatetime: 2025-10-09T00:00:00Z
slug: functional-thinking-go-lambda-calculus
featured: false
draft: false
...
```go
// [M + N] → λa.λb.((M a)((N a) b))
func Add(M, N func(int) func(int) int) func(int) func(int) int {
...
// [M × N] → λa.(M (N a))
func Multiply(M, N func(int) int) func(int) int {
    return func(a int) int {
        return M(N(a))
    }
}
// [M ^ N] → N M
func Power(M func() int, N func(func() int) int) int {
    return N(M)
}
```
These Go functions illustrate how Church’s abstract formulas can be represented as composable, higher-order functions that operate on other functions, capturing computation patterns in a reusable way.

### 2. **Bound vs Free Variables: Proper Scope Management**  
> An occurrence of a variable x in a well-formed formula II. is bound or free according as it is or is not an occurrence in a well-formed part of II. of the form $$(λxN)$$… no occurrence of a variable in a well-formed formula is both bound and free.
...
- Bound variables → local variables and function parameters: Variables declared inside a function or closure are only visible there, preventing unintended interference with other parts of the program.
- Free variables → captured variables in closures or global state: Variables referenced from an outer scope but not declared locally. These need careful handling to avoid hidden dependencies or side effects.
...
> “…if $$ ((\lambda x_j N_j) W_j) $$ are parts of 𝐴 A, then a number 𝑚 m can be found such that any sequence of contractions on these parts will terminate after at most 𝑚 m contractions… the result is unique up to the order of applying the reduction rules.”
...
```
Here, the recursion always reaches the base case, analogous to Church’s guarantee that every sequence of reductions reaches a unique final formula.
